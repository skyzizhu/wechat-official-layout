 ...
 */
function convertPlainTextToMarkdown(text, options) {
    if (!text || text.trim().length === 0)
        return '';
    ...
    while (i < rawLines.length) {
        ...
        const isCodeStart = /^(?:const|let|var|function|import|export|class|def|public|private|protected|static|void|async|interface|type|enum|readonly)\s/.test(trimmed) ||
        ...
        if (isCodeStart) {
            ...
            while (j < rawLines.length) {
                ...
                consecutiveEmpty = 0;
                const isStillCode = /^(?:const|let|var|function|import|export|class|interface|type|enum|def|return|if|else|for|while|switch|case|try|catch|finally|console|print|readonly)\b/.test(nextTrimmed) ||
                    /^[}\]\);,]/.test(nextTrimmed) ||
    ...
    for (let idx = 0; idx < blockProcessedLines.length; idx++) {
        ...
        const isH3Pattern = /^\d+\.\d+[\s、.．]+\S+/.test(trimmed) ||
        ...
        const isCaptionPattern = /^\*?(?:▲\s*|\[)?(?:图|表|Figure|阶段)\s*[\dA-Za-z\-]+/i.test(trimmed) ||
            /^\*?▲\s*.+$/i.test(trimmed) ||
            /^\*?注[：:]/.test(trimmed) ||
